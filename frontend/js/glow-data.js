...
const DB = {
  ...
  // ── CHAT SYSTEM ───────────────────────────
  getChats() {
    const raw = localStorage.getItem('bz_chats');
    return raw ? JSON.parse(raw) : [];
  },
  saveChats(chats) {
    localStorage.setItem('bz_chats', JSON.stringify(chats));
    document.dispatchEvent(new CustomEvent('chatsUpdated', { detail: chats }));
  },
  ...
  getPaymentNotifications() {
    const raw = localStorage.getItem('bz_payment_notifications');
    return raw ? JSON.parse(raw) : [];
  },
  savePaymentNotification(notification) {
    const notifications = this.getPaymentNotifications();
    notifications.push({
      id: 'pay-' + Date.now(),
      ...notification,
      createdAt: new Date().toISOString(),
    ...
    localStorage.setItem('bz_payment_notifications', JSON.stringify(notifications));
    document.dispatchEvent(new CustomEvent('paymentNotificationsUpdated', { detail: notifications }));
  }
};
