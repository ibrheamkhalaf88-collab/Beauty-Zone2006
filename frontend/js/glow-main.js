...
document.addEventListener('DOMContentLoaded', () => {
      ...
      rows.forEach((rowProducts, rowIdx) => {
        ...
        rowProducts.forEach((p, prodIdx) => {
          ...
          if (rowIdx === 0) {
            ...
            if (img) {
              img.style.clipPath = 'none !important';
  ...
  function openBankTransferModal() {
    ...
    document.getElementById('bt-order-id').textContent = orderId;
    document.getElementById('bt-total').textContent = total + ' ₪';
    document.getElementById('bt-amount').value = total;

    bankTransferModal.style.display = '';
    bankTransferModal.classList.remove('hidden');
    gsap.from('.bank-transfer-box', { y: 40, opacity: 0, duration: 0.4, ease: 'back.out' });

    window.currentBankOrderId = orderId;
    window.currentBankTotal = total;
    closeCart();
  }

  function closeBankTransferModal() {
    if (!bankTransferModal) return;
    gsap.to('.bank-transfer-box', { y: 20, opacity: 0, duration: 0.3, onComplete: () => {
      bankTransferModal.classList.add('hidden');
      bankTransferModal.style.display = 'none';
    }});
  }

  document.getElementById('checkout-bank')?.addEventListener('click', openBankTransferModal);
  document.getElementById('bank-transfer-close')?.addEventListener('click', closeBankTransferModal);
  bankTransferModal?.addEventListener('click', e => { if (e.target === bankTransferModal) closeBankTransferModal(); });
  ...
  if (bankTransferForm) {
    bankTransferForm.addEventListener('submit', async (e) => {
      ...
      const screenshotInput = document.getElementById('bt-screenshot');
      const cart = DB.getCart();
      ...
      if (screenshotInput.files[0]) {
        screenshotData = await toBase64(screenshotInput.files[0]);
      }

      const currentUser = DB.getCurrentUser();

      const order = {
        id: window.currentBankOrderId,
        items: cart,
        total: amount,
        status: 'pending',
        paymentMethod: 'bank_transfer',
        userId: currentUser ? (currentUser.id || currentUser.phone) : null,
        userName: currentUser ? currentUser.name : null,
        userPhone: currentUser ? currentUser.phone : null,
        userLocation: currentUser ? currentUser.location : null,
        transferDetails: {
          name,
          ref,
          amount,
      ...
      document.getElementById('success-note').textContent = 'تم استلام طلبك! سنقوم بمراجعة التحويل والتواصل معك قريباً 💬';
      showSuccess(order.id);

      showToast('✅ تم إرسال بيانات التحويل بنجاح!');
    });
  }
  ...
  if (regBtn) {
    regBtn.addEventListener('click', () => {
      ...
      const location = regLocation ? regLocation.value.trim() : '';
      ...
      const newUser = { name, phone, email, pass, location, role: 'user', createdAt: new Date().toISOString() };
      DB.saveUser(newUser);
...
(function initPWA() {
  ...
  const chatPaymentToggle = document.getElementById('chat-payment-toggle');
  const chatPaymentSection = document.getElementById('chat-payment-section');
  const chatPaymentNotif = document.getElementById('chat-payment-notif');
  const sendPaymentBtn = document.getElementById('send-payment-notif');

  function openChatModal() {
    if (!chatModal) return;
    chatModal.style.display = '';
    chatModal.classList.remove('hidden');
    gsap.from('.chat-modal-box', { y: 40, opacity: 0, duration: 0.4, ease: 'back.out' });
    loadChatMessages();
  }

  function closeChatModal() {
    if (!chatModal) return;
    gsap.to('.chat-modal-box', { y: 20, opacity: 0, duration: 0.3, onComplete: () => {
  ...
  function loadChatMessages() {
    ...
    const chat = DB.getUserChat(currentUser.id || currentUser.phone);
    if (!chat || !chat.messages.length) {
      chatMessages.innerHTML = `
        <div class="chat-empty-state">
          <p>👋 أهلاً بك ${currentUser.name || 'مجتمعنا'}!</p>
    ...
    chat.messages.forEach(msg => {
      ...
      msgEl.className = `chat-message ${isUser ? 'user' : 'admin'}`;
      msgEl.innerHTML = `
        <div class="chat-bubble">${msg.text}</div>
        <div class="chat-time">${new Date(msg.timestamp).toLocaleTimeString('ar', { hour: '2-digit', minute: '2-digit' })}</div>
      `;
  ...
  function sendMessage() {
    ...
    let chat = DB.getUserChat(userId);
    if (!chat) chat = DB.getOrCreateChat(userId);

    DB.addMessage(chat.id, { text, sender: 'user', senderName: currentUser.name || 'مستخدم' });
    chatInput.value = '';
  ...
  if (chatPaymentToggle) {
    chatPaymentToggle.addEventListener('click', () => {
      chatPaymentSection.style.display = chatPaymentSection.style.display === 'none' ? '' : 'none';
    });
  }
  ...
  if (sendPaymentBtn) {
    sendPaymentBtn.addEventListener('click', () => {
      ...
      DB.savePaymentNotification(notification);
      document.getElementById('payment-amount').value = '';
      showToast('✅ تم إرسال إشعار الدفع بنجاح!');
    });
  }
  ...
  function openMyOrdersModal() {
    ...
    myOrdersModal.style.display = '';
    myOrdersModal.classList.remove('hidden');
    gsap.from('.my-orders-box', { y: 40, opacity: 0, duration: 0.4, ease: 'back.out' });
    renderMyOrders();
  }

  function closeMyOrdersModal() {
    if (!myOrdersModal) return;
    gsap.to('.my-orders-box', { y: 20, opacity: 0, duration: 0.3, onComplete: () => {
  ...
  function renderMyOrders() {
    const currentUser = DB.getCurrentUser();
    if (!currentUser) {
      myOrdersList.innerHTML = '<div class="no-orders"><p>يرجى تسجيل الدخول أولاً</p></div>';
      return;
    }

    // Get all orders and filter by user (for now, get all orders since we don't have userId on orders)
    const allOrders = DB.getOrders();
    // For WhatsApp orders, user info isn't stored. For bank transfer orders, we have user details.
    // Let's get bank transfer orders that match user phone or email
    const userOrders = allOrders.filter(o => {
      if (o.paymentMethod === 'bank_transfer') {
        return o.transferDetails && (o.transferDetails.phone === currentUser.phone || o.transferDetails.email === currentUser.email);
      }
      // For WhatsApp orders - show all since they don't have user identification
      return true;
    }).reverse();
    ...
    if (!userOrders.length) {
      myOrdersList.innerHTML = `
        <div class="no-orders">
          ...
          <p style="font-size:.85rem;margin-top:8px">ابدأي بالتسوق الآن!</p>
        </div>
      `;
      return;
    }

    myOrdersList.innerHTML = userOrders.map(o => {
      const statusClass = o.status === 'مكتمل' ? 'completed' :
                          o.status === 'ملغي' ? 'cancelled' :
                          o.status === 'قيد التجهيز' || o.status === 'تم الشحن' ? 'processing' : 'pending';

      const statusLabel = o.status === 'جديد' ? 'بانتظار المراجعة' :
                          o.status === 'قيد التجهيز' ? 'قيد التجهيز' :
                          o.status === 'تم الشحن' ? 'تم الشحن' :
                          o.status === 'مكتمل' ? 'تم التأكيد' :
                          o.status === 'ملغي' ? 'ملغي' : o.status || 'بانتظار المراجعة';
        ...
        itemsHtml = '<div class="order-card-items">' +
          o.items.map(item => `
            <div class="order-item">
              <span class="order-item-name">${item.name || item.nameAR || 'منتج'}</span>
              <span class="order-item-price">${item.qty || 1} × ${item.price} ₪</span>
            </div>
          `).join('') +
          '</div>';
      }
      ...
      let transferDetails = '';
      if (o.paymentMethod === 'bank_transfer' && o.transferDetails) {
        transferDetails = `
          <div class="transfer-details">
            <p><strong>🏦 نوع الدفع:</strong> تحويل بنكي</p>
            <p><strong>المحوّل:</strong> ${o.transferDetails.name}</p>
            <p><strong>رقم المرجع:</strong> ${o.transferDetails.ref}</p>
            <p><strong>المبلغ:</strong> ${o.transferDetails.amount} ₪</p>
          </div>
        `;
      } else {
        transferDetails = `
          <div class="transfer-details">
            <p><strong>📱 نوع الدفع:</strong> عبر واتساب</p>
          </div>
        `;
      }
      ...
      return `
        <div class="order-card">
          ...
          <div class="order-card-footer">
            ...
            <span class="order-date">${new Date(o.date).toLocaleDateString('ar')}</span>
          </div>
        </div>
      `;
    }).join('');
  ...
  document.getElementById('my-orders-close')?.addEventListener('click', closeMyOrdersModal);
