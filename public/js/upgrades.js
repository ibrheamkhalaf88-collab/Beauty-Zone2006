      ...
      items.forEach(p => {
        ...
        row.innerHTML = `
          ...
          <div class="ac-info">
            ...
            <p class="ac-cat">${p.sku ? '<span style="font-weight:700;color:var(--plum)">' + p.sku + '</span> · ' : ''}${labels[p.category] || p.category}</p>
          </div>
        ...
        row.addEventListener('click', () => {
          ...
          // Find and scroll to the product card first
          const productCard = document.querySelector(`.product-card[data-id="${p.id}"]`);
          if (productCard) {
            productCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
            // Highlight briefly
            productCard.style.transition = 'box-shadow 0.3s, transform 0.3s';
            productCard.style.boxShadow = '0 0 0 3px var(--gold), 0 8px 30px rgba(196,163,90,0.3)';
          ...
          // Open product modal after scroll
          setTimeout(() => {
            if (typeof openProductModal === 'function') openProductModal(p.id);
            else if (typeof openQuickView === 'function') openQuickView(p.id);
          }, 500);
        });
        box.appendChild(row);
      });
